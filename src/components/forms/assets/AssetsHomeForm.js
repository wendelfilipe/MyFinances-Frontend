    ...
    return (
        <div>
            {assetsEachType.map(a => {
                ...
                if (matchedUserAsset.sourceTypeAssets === 2 && matchedUserAsset) {
                    return (
                        <div key={a.id} className="card mb-3">
                            <div className="card-header d-flex justify-content-between align-items-center">
                                Código da Ação: {a.codName}
                                <span className="text-end">Total: R$ {(matchedUserAsset.averegePrice).toFixed(2)}</span>
                            </div>
                            <div className="card-body">
                                ...
                                <p className="card-title d-inline-block me-3">Porcetagem: {(((matchedUserAsset.amount * matchedUserAsset.averegePrice)*100)/totalAssets).toFixed(2)}%</p>
                            </div>
                        </div>
                    );
                } 
